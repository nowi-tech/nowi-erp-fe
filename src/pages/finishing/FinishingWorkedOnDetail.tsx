...
export default function FinishingWorkedOnDetail() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [lotRes, receipts, scraps] = await Promise.all([
        ...
        // filtering so finishing_master can see upstream activity too.
          ...
          <>
            ...
            <div className="rounded-[14px] bg-[var(--color-surface)] shadow-[0_1px_2px_rgba(15,26,54,0.04)] p-4">
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                <SummaryCell
                  label={t('finishing.lot.forwarded', {
                ...
                />
                <SummaryCell
                  label={t('finishing.lot.scrap', { defaultValue: 'Scrap' })}
                  value={summary.scrapped}
                  tone="scrap"
                />
                <SummaryCell
                  label={t('admin.locator.filters.rework', {
                    defaultValue: 'Rework',
                  })}
...
function SummaryCell({
  ...
  label: string;
  value: number;
  tone: 'finish' | 'scrap' | 'rework';
}) {
  // `finish` uses the floor app's primary token (the finishing section
  // was deliberately aligned off the vermilion stage accent); scrap /
  // rework keep their semantic status tokens (shared with stitching).
  const toneClass =
    tone === 'finish'
